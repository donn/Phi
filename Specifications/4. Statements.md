...
Optionally, a user may elect to specify a bus width. The bus width is provided as a range. Ranges are declared as either **standalone numbers** or **software expressions** with two dots (**..**) in between.
...
Again, optionally, a user may elect to specify an **array** declaration. Like in C, an array is not a fundemantal part of the type and instead has to do with the variable itself. Array declarations, unlike bus declarations, use only a software expression or a standalone number: ranges are not allowed.
