...
Optionally, a user may elect to specify a bus width. The bus width is provided as a range. Ranges are declared as either **numbers** or **software expressions** with two dots (**..**) in between.
...
Users may chain an if statement with if and else if.
 ...
 All elaboration for if statements happens at compile time. Therefore, `if` statements cannot use **hardware expressions** outside of an async block and declarations are not allowed inside if statements.
...
There are some key requirements for async blocks:
    ...
    C. A wire cannot be a function of itself.
...
```
***Snippet 11: Example async blocks.  The second is not allowed as assignment does not occur under all conditions. The third is not allowed as the value of a is a function of itself.***
...
#### 4.3.3.1 The `if` statement inside async blocks
Inside an async block, the if statament behaves differently in that the condition must be a 1-bit **bus** expression.

Additionally, `else` blocks are almost always a requirement, as per requirement B of async blocks. You can also choose an initial assignment if it suits you.
...
```phi
    ...
    @elseRetain async {}
```
***Snippet 13: Examples for annotations on async blocks.***
