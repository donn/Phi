...
Variables can be coupled with a bus declaration, i.e. `Var[31..0]`, to declare a variable. They may be from 1 to 65536 bits long. (Note that any value with greater than 64-bits will compile and simulate a little bit slower.)
...
### 1.4.1 $
* Type: Function call
* Operands: Many software expressions of variable widths
* Result: One X-bit software expression

Syntax: `$name(list, of, arguments...)`;

Invocation of a system call as shall be detailed later in the language. The result depends on the system call. Note that it only uses software expressions and hardware expressions cannot be used as an argument.
