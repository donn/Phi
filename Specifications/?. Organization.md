...
```
module ClockDivider(input: Input, reset: Input, output: Output, cycleCount: Output) {
    namespace Dividing {
...
An interface is a set of ports a module may choose to comply with. A module can comply with one or more interfaces, and add ports of its own (so long as they do not neglect output ports they have complied to.)

As an example, when designing an SoC for a specific bus, typically any module has to have a specific set of ports.
...
```phi
namespace Example {
    module IO: AHBLite<width: (32)> (output: Output) {
